...
```bash
pnpm i -D mini-ssg-zen windicss
```
...
+    "dev": "ssg --watch",
+    "build": "ssg",
+    "windi:dev": "windicss './dev/**/*.html' -mto './dev/static/windi.css' --dev --style",
+    "windi:build": "windicss './dev/**/*.html' -mto './dev/static/windi.css' --style"
  },
  "keywords": [],
  "author": "",
  "license": "ISC",
...
```bash
pnpm windi:dev & pnpm dev
```

Mode build:

```bash
pnpm windi:build && pnpm build
```
